        ...
        <div id="boisson-details-container" style="display: none;">
            ...
            <table id="available-drinks-table">
                <thead>
                    <tr>
                        <th>Nom Boisson</th>
                        <th><i class='bx bx-image' style='color: darkgreen; font-size: 24px;'></i></th>
        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo '<tr>';
                    echo '<td style="text-decoration: none;">' . htmlspecialchars($row['nom_boisson']) . '</td>';
                    echo '<td><img src="' . htmlspecialchars($imagePath) . '" alt="Boisson Image" style="width: 50px; height: 50px; border-radius: 5px;"></td>';
                    echo '<td style="color:' . htmlspecialchars($quantiteColor) . ';">' . htmlspecialchars($row['quantite_stock']) . '</td>';
                    echo '<td>' . htmlspecialchars($row['prix_achat']) . ' FBU</td>';
                    echo '</tr>';
                }
            } else {
        ...
        <div id="kitchen-details-container" style="display: none;">
            ...
            <table id="available-kitchen-table">
                ...
                try {
                    ...
                    // Query to fetch data including picture and availability columns, ordered alphabetically by nom_boisson
                    $query = "SELECT nom_plat, availability, prix_achat FROM gestion_stock_kitchen ORDER BY nom_plat ASC";
                    $stmt = $conn->prepare($query);
                    $stmt->execute();
                    ...
                    if ($stmt->rowCount() > 0) {
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            // Set image path (assuming images are stored in 'G_boisson/uploads/')
                            $imagePath = !empty($row['picture']) ? "G_boisson/uploads/" . htmlspecialchars($row['picture']) : "G_boisson/uploads/default.jpg";
                            
                            echo '<tr>';
                            echo '<td>' . htmlspecialchars($row['nom_plat']) . '</td>';
                            echo '<td style="color:' . $availabilityColor . ';">' . htmlspecialchars($row['availability']) . '</td>';
                            echo '<td>' . htmlspecialchars($row['prix_achat']) . ' FBU</td>';
                            echo '</tr>';
                        }
                    } else {
